...
export function AddProductDialog({ open, onOpenChange }: AddProductDialogProps) {
  ...
  const [vendors, setVendors] = useState([
    { id: 1, name: "OM X Limited" },
    { id: 2, name: "Ama Chori ko Achar" },
    { id: 3, name: "Baje ko Sekuwa" },
    { id: 4, name: "Denim Pants" }
  ])
  ...
  function addNewVendor() {
    if (newVendor.trim() && !vendors.some(vendor => vendor.name.toLowerCase() === newVendor.trim().toLowerCase())) {
      const newId = Math.max(...vendors.map(vendor => vendor.id), 0) + 1
      const updatedVendors = [...vendors, { id: newId, name: newVendor.trim() }]
      setVendors(updatedVendors)
      form.setValue("vendor", newVendor.trim())
      setNewVendor("")
  ...
  function addNewSection() {
    if (newSection.trim() && !sections.some(sec => sec.name.toLowerCase() === newSection.trim().toLowerCase())) {
      const newId = Math.max(...sections.map(sec => sec.id), 0) + 1
  ...
  return (
    <>
      {/* Add Vendor Dialog */}
      <Dialog open={vendorPopup} onOpenChange={setVendorPopup}>
